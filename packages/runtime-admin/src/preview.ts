...
/**
 * As-of preview (S6): an editor picks a moment and browses the public site
 * as it was (or, with scheduled content, will be) at that moment. Rides on
 * Next draft mode: the bypass cookie makes the prerendered pages render
 * dynamically, and our own cookie carries the chosen moment. Pages stay
 * fully static for normal visitors — draftMode() is the only check, and it
 * does not make a route dynamic. Entering and leaving is the admin's job
 * (admin-server: `previewEnter`, `previewExit`).
 */
