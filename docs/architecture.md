...
| **Bibliotheek** | herbruikbare onderdelen die een site *kiest*: widgets (schema + viewer + optioneel editor), plugins (nog geen package), mogelijk basisthema's/presets | `packages/widgets-standard` (39 standaardwidgets: schema's en viewers); MusicBrains domeinwidgets en alle editors nog in `sites/musicbrain/src/widgets/` | de engine-contracten (`WidgetTypeDef`, `ContentStore`), nooit een site |
...
| grenscontrole | `scripts/check-boundaries.mjs` (vóór `npm test`) | geen package importeert uit een site (`@/`, `sites/`); `content-core` en `extension-api` importeren geen React of Next |
| plugins in de instantie | `packages/extension-api/test/imprint.test.ts` | een plugin brengt typen mee; ongeldige naam, ontbrekende versie, dubbel geconfigureerd en een bestaand type worden bij `defineImprint` geweigerd met de naam van de plugin |
