...
| 5 — Eerste echte plugins | **bezig**: stap 1–3 klaar — contenttyperegister, plugincontract (`ImprintPluginCore`/`ImprintPlugin`, drie haken), `@imprint/plugin-planning` aan in MusicBrain met één regel; de kern kent planning niet meer. Rest: stap 4 (wiki) en 5 (exitproef). Ontwerp in [fase-5-plugins.md](fase-5-plugins.md) | |
| 6 — Hardening | open | |
