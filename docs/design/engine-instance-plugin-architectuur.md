...
| 4 — Studio en widgeteditors | **bezig**: stap 1–3 klaar (layoutoperaties, drafts per instantie, studio en actions in `runtime-admin`, `AdminContext.studio` met viewers/chrome/editor); exitcriterium gehaald — dezelfde studio bewerkt pagina's op MusicBrain én de Imprint-site. Rest: stap 4, standaard-widgeteditors naar `widgets-standard` | architecture.md §5 |
