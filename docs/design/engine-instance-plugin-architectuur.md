...
| 3 — Generieke adminbasis | **bezig**; ontwerp en besluiten vastgelegd, open vragen in §11.3 daarvan. Stap 1 klaar: gebruikers op Postgres, secrets via de config, contenttypecatalogus beschikbaar/actief. Stap 2 klaar: browsertests van alle admin-flows. Stap 3 klaar: poortje in AuthZEN-vorm, `access` op alle inhoud, `/members/…`. Stap 4 klaar: `AdminContext` en de generieke clientcomponenten in `runtime-admin/admin`. Stap 5 klaar: alle admin-schermen en actions in `runtime-admin/admin-server`, menu uit de catalogus plus `contributions`. Volgende: stap 6 (MusicBrain volledig uit het package, planning en wiki als bijdragen) en 7 (de Imprint-site krijgt de admin) | [fase-3-admin-toegang-tijdreizen.md](fase-3-admin-toegang-tijdreizen.md) |
