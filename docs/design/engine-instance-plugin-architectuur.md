...
| 2 — Renderer en standard widgets | **klaar**: renderer vastgelegd in golden HTML (`933de2c`); renderer en layouthelpers naar `@imprint/runtime-admin` (`231abc1`); viewers krijgen een `WidgetContext` (`d04aa40`); twintig standaardwidgets naar `@imprint/widgets-standard` (`0069778`); de Imprint-site rendert databasepagina's met acht daarvan | architecture.md §3, §7 en §8; backlog |
| 3 — Generieke adminbasis | **klaar** (stap 0–7, september 2026): gebruikers op Postgres, secrets en catalogus in de config, browsertests, poortje in AuthZEN-vorm met `access` en `/members`, `AdminContext`, alle admin-schermen en -actions plus sessie en preview in `@imprint/runtime-admin`; de Imprint-site heeft de admin met dertien dunne bestanden. Eerder:  ontwerp en besluiten vastgelegd, open vragen in §11.3 daarvan. Stap 1 klaar: gebruikers op Postgres, secrets via de config, contenttypecatalogus beschikbaar/actief. Stap 2 klaar: browsertests van alle admin-flows. Stap 3 klaar: poortje in AuthZEN-vorm, `access` op alle inhoud, `/members/…`. Stap 4 klaar: `AdminContext` en de generieke clientcomponenten in `runtime-admin/admin`. Stap 5 klaar: alle admin-schermen en actions in `runtime-admin/admin-server`, menu uit de catalogus plus `contributions`. | [fase-3-admin-toegang-tijdreizen.md](fase-3-admin-toegang-tijdreizen.md) |
| 4 — Studio en widgeteditors | **klaar** (september 2026): layoutoperaties en drafts per instantie, studio en actions in `runtime-admin`, `AdminContext.studio` met viewers/chrome/editor, standaard-widgeteditors in `widgets-standard/editors`. Exitcriterium gehaald: dezelfde studio bewerkt pagina's op MusicBrain én de Imprint-site, elk met eigen chrome en widgetset | architecture.md §3 en §5 |
| 5 — Eerste echte plugins | **klaar** (september 2026): contenttyperegister (`ContentType` open), plugincontract met drie haken, `@imprint/plugin-planning` en `@imprint/plugin-wiki`; MusicBrain zet ze aan met één regel elk, de Imprint-site heeft ze niet en kent de typen niet (exitproef). Ontwerp in [fase-5-plugins.md](fase-5-plugins.md) | architecture.md §5 |
| 6 — Hardening | **bezig**: conflictvrije registratie van plugins en typen met bruikbare fouten bij opstarten, grenscontrole op packages in `npm test`, Extensions-overzicht op het dashboard. Open (vragen aan Mark): engine-compatibiliteitscontract per plugin, healthcheck-endpoint, content die naar een uitgeschakelde widget verwijst (open vraag 8 van Fase 3), migratievolgorde van plugins | architecture.md §8 |
